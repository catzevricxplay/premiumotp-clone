...
const APP_CONFIG = {
  ...
  MASTER_KEY: "", // Empty = each user uses his own key from signup/login
  ACTIONS: {
...
function getCurrentUser() {
  try {
    ...
    if (email || name) {
      return { email: email || '', firstName: name ? name.split(' ')[0] : '', lastName: name ? name.split(' ').slice(1).join(' ') : '', balance: balance || '0.00' };
    }
    return null;
  } catch { return null; }
}
